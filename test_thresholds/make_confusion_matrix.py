...
def confusion_matrix(hf_group, hf_thresh):

    hf_keys    = list(hf_thresh.keys())

    n=0 # sza, vza, raz, scene_id
    #print(threshold_path[n+7:n+9])
    OLP =[int(threshold_path[n+7:n+9])  ,\
          int(threshold_path[n+14:n+16]),\
          int(threshold_path[n+21:n+23]),\
    ...
    OLP = np.zeros((len(hf_keys), 6))
    for i, bins in enumerate(hf_keys):
        OLP[i,:] = [int(threshold_path[n+7:n+9])  ,\
                    int(threshold_path[n+14:n+16]),\
                    int(threshold_path[n+21:n+23]),\
                    int(threshold_path[n+45:n+47]) ]
    ...
    #grab obs, CM, and thresholds
    for i, bin in enumerate(hf_keys):
        data = hf_thresh[bin][()]
        cloud_mask = data[:, 0]
        obs        = data[:, 1:]
    ...
    for i in range(num_points):
        ...
        for j in range(7):
            #this is for whiteness since 0 is whiter than 1 and not applied over snow/ice or sunglint
            if j==0 and thresholds[0] <= obs[i,0] and (OLP[-2]!=13 and OLP[-2]!=14):
                cloud_mask_MAIA[i] = 0
            #DTT for NDxI. Must exceed 0
            #NDVI everything but snow
            elif j==1 and OLP[-2]!=14 and DTT_NDxI[i,0] >= 0:
                cloud_mask_MAIA[i] = 0
            #NDSI only over snow
            elif j==2 and OLP[-2]==14 and DTT_NDxI[i,1] >= 0:
                cloud_mask_MAIA[i] = 0
            #VIS, NIR, Cirrus. Must exceed thresh
            #VIS applied only over land
            elif j==3 and OLP[-2]<=11 and thresholds[3] >= obs[i,3]:
                cloud_mask_MAIA[i] = 0
            #NIR only applied over water (no sunglint)
            elif j==4 and OLP[-2]==12 and thresholds[4] >= obs[i,4]:
                cloud_mask_MAIA[i] = 0
            #SVI applied over all surfaces when over 0. Must exceed thresh
            elif j==5 and obs[i,5] >= 0.0  and thresholds[5] >= obs[i,5]:
                cloud_mask_MAIA[i] = 0
...
if __name__ == '__main__':
    ...
    thresh_path  = home + 'thresholds.hdf5'

    with h5py.File(grouped_path) as hf_group,\
         h5py.File(thresh_path) as hf_thresh:

        confusion_matrix(hf_group, hf_thresh)
