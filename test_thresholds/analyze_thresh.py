...
def make_obs_hist_by_group(obs):
    ...
    for i, gf in enumerate(group_files):
        with h5py.File(gf, 'r') as hf_gf:
            ...
            if num_sample_clear > 500 and num_sample_cloud >500:
                plt.hist(bins=bin_edges_clear[:-1], hist_clear, 'b', label='clear')
                plt.hist(bins=bin_edges_cloud[:-1], hist_cloud, 'r', label='cloudy')
                plt.legend()
