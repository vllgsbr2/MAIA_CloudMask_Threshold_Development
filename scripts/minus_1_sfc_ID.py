...
def make_SID_MCM_rdy_old(home_og, home):
    '''
    ...
    sfc_ID_paths = [x for x in sfc_ID_paths if x[:12] == 'surfaceID_Lo']
    ...
    # 0  - Water
    # 1  - darkest; Coastline; invalid retrievals
    # 12 - Brightest

    #modify the surface ID in memory
    sfc_IDs_mod = np.copy(sfc_IDs) - 1
    #set sfcID -1 to 12
    sfc_IDs_mod[sfc_IDs ==-1]  = 12

    #copy files into new destination
    os.system('sudo cp -f {}/surfaceID_Lo* {}/'.format(home_og, home))
