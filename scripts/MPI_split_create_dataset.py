...
def get_rad(filename_MOD_02, fieldname):
    rad_or_ref            = True
    ...
    return radiance_250_Aggr1km, radiance_500_Aggr1km, radiance_1KM

def get_ref(filename_MOD_02, fieldname):
    rad_or_ref              = False
    ...
    return reflectance_250_Aggr1km, reflectance_500_Aggr1km, reflectance_1KM

def get_sunview_geo(filename_MOD_03):
    ...
    return lat, lon, solarZenith, sensorZenith, solarAzimuth, sensorAzimuth

def get_CM(filename_MOD_35):
    #calculate cloudmask
    data_SD           = get_data(filename_MOD_35, 'Cloud_Mask', 2)
    data_SD_bit       = get_bits(data_SD, 0)
    data_decoded_bits = decode_byte_1(data_SD_bit)

    return data_decoded_bits

def get_CM_tests(filename_MOD_03, filename_MOD_35):
    data_SD                  = get_data(filename_MOD_35, 'Cloud_Mask', 2)
    decoded_cloud_mask_tests = decode_tests(data_SD, filename_MOD_35)

    return decoded_cloud_mask_tests

def get_regrid_row(source_lat, source_lon, target_lat, target_lon, row_mesh):
    regrid_row_idx = regrid_MODIS_2_MAIA(np.copy(source_lat),\
                                         np.copy(source_lon),\
                                         np.copy(target_lat),\
                                         np.copy(target_lon),\
                                         np.copy(row_mesh).astype(np.float64)).astype(np.int)

    return regrid_row_idx

def get_regrid_col(source_lat, source_lon, target_lat, target_lon, col_mesh):
    regrid_col_idx = regrid_MODIS_2_MAIA(np.copy(source_lat),\
                                         np.copy(source_lon),\
                                         np.copy(target_lat),\
                                         np.copy(target_lon),\
                                         np.copy(col_mesh).astype(np.float64)).astype(np.int)
    return regrid_col_idx

def save_and_subset(rad, ref, sunview_geo, CM, CM_tests, regrid_row, regrid_col):
